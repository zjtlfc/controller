...
struct      CanRepeatTran     CanRepeatBuff[CAN_REPEAT_BUFFER_SIZE]=                /*CAN循环发送缓冲区定义*/
      {     /*   周期  当前时间     CAN_ID           数据长度 动作 ID类型  通道     发送数量     数据值    */
                  ...
                  50,   50,   BMS_ERROR_MESSAGE,           8, 0X55,  1,  SYSTEM_CAN, 0,       {0,0,0,0,0,0,0,0},                    /*BMS在内CAN中上报报警信息,ID=0X012EE511(19410222)*/
                  1000, 1000, BMS_AUX2_MESSAGE,            8, 0X55,  1,  SYSTEM_CAN, 0,       {0,0,0,0,0,0,0,0},                    /*BMS运行辅助信息,ID=0X01282D2E(19410222)*/
                  500,  500,  BMS_CHARGE_MESSAGE,          8, 0X55,  1,  SYSTEM_CAN, 0,       {0,0,0,0,0,0,0,0},                    /*BMS发出与充电信息相关数据,ID=0X0129DAD7(19520215)*/
                  50,   50,   BMS_BALANCE_MESSAGE,         8, 0X55,  1,  SYSTEM_CAN, 0,       {0,0,0,0,0,0,0,0},                    /*BMS系统动态平衡相关信息,ID=0X012E4AC3(19811011)*/
      };          
...
unsigned	short		SoundOutputDriver1[SOUND_DRIVE_BUFF_SIZE+5];	/*驾驶员座椅声音输出1（数据：0—1666）*/
unsigned	short		SoundOutputDriver2[SOUND_DRIVE_BUFF_SIZE+5];	/*驾驶员座椅声音输出2（数据：0—1666）*/	
...
unsigned	int	TempNumber;
	...
	{
		SoundOutputDriver1[TempNumber]=833;
		SoundOutputDriver2[TempNumber]=833;
	};
...
*   0                  	函数调用失败或无数据传输
======================================================================================================================================================*/
unsigned int OutputLRData(unsigned short *LeftOutputPtr,unsigned int LefuOutputSize,unsigned short *RightOutputPtr,unsigned int RightOutputSize)
{
int	ReturnCode;
unsigned int TempNumber,TempSoundValue;
	ReturnCode=0;
	if((LefuOutputSize <=SOUND_DATA_BUFF_SIZE) &&(RightOutputSize <=SOUND_DATA_BUFF_SIZE))	/*传输的数据长度有效*/
	{
		SoundDataLength=SoundDataLength + LefuOutputSize;				/*未播放的生硬数据长度*/
		for(TempNumber=0;TempNumber<LefuOutputSize;TempNumber++)	/*将左右声道数据发送的输出缓冲区*/
		{
		...
		SoundBottomPoint=(SoundBottomPoint+LefuOutputSize) %SOUND_DRIVE_BUFF_SIZE;		
		if(SoundDriveMark==0)	/*声音播放标志，1：播放；0：未播放*/
		{
			if(SoundDataLength > 1500)	/*有足够的未播放数据可以播放*/
			{
				SoundDriveMark=1;				/*声音播放标志，1：播放；0：未播放*/
				ENABLE_EN_GR1;				/*设置主驾驶座椅驱动器使能，PTD14=1*/
				ENABLE_EN_GR4;				/*设置后排成员2座椅驱动器使能，PTE6=1*/
				Fmt0Dma0Tran((unsigned char *)(&SoundOutputDriver1[0]),SOUND_DRIVE_BUFF_SIZE);		/*FMT0，DMA0数据传输*/
				Fmt0Dma1Tran((unsigned char *)(&SoundOutputDriver2[0]),SOUND_DRIVE_BUFF_SIZE);		/*FMT0，DMA0数据传输*/
			};
		}else
		{
