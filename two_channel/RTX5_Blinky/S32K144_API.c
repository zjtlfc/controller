...
void DmaInitial(void)
{
      ...
      DMAMUX->CHCFG[1]=0;          /*DMA通道1为前排乘员座椅驱动数据接收,使能DMA通道1*/
      ...
      DMAMUX->CHCFG[1]=DMAMUX_CHCFG_ENBL(1) | FTM1_CH1_DMA_SROUCE;     /*DMA通道1为前排乘员座椅驱动数据接收,使能DMAFTM1模块通道1 DMA源*/
...
//      DMAMUX->CHCFG[9]= DMAMUX_CHCFG_ENBL(1) | DMA_SPI1_TRAN_SOURCE;    /*SPI0通道TX DMA源*/
      ...
      DMA->DCHPRI[1]=DMA_DCHPRI_CHPRI(1)| DMA_DCHPRI_DPA(1)| DMA_DCHPRI_ECP(1);         	/*前排乘员座椅驱动通道DMA优先级*/
...
void FTM1_PWM_init(unsigned int Channel,unsigned int duty)
{
	...
	switch(Channel)
	{
		...
		break;
		case	5:
			FTM1->CONTROLS[5].CnSC = FTM_CnSC_MSB_MASK | FTM_CnSC_ELSB_MASK;  	/* FTM0 ch6: edge-aligned PWM, low true pulses 		*/
															/* CHIE (Chan Interrupt Ena) = 0 (default) 			*/
															/* MSB:MSA (chan Mode Select)=0b10, Edge Align PWM		*/																																			/* ELSB:ELSA (chan Edge/Level Select)=0b10, low true 	*/
		...
		break;
		case	7:
			FTM1->CONTROLS[7].CnSC = FTM_CnSC_MSB_MASK | FTM_CnSC_ELSB_MASK ;  	/* FTM0 ch6: edge-aligned PWM, low true pulses 		*/
															/* CHIE (Chan Interrupt Ena) = 0 (default) 			*/
															/* MSB:MSA (chan Mode Select)=0b10, Edge Align PWM		*/																																			/* ELSB:ELSA (chan Edge/Level Select)=0b10, low true 	*/
...
void Fmt0Dma1Tran(unsigned char *Tran,unsigned int DataSize)				/*FMT0，DMA1数据传输*/
{
//int   TempSize;
      if(DataSize>0)    /*SPI发送的数据必须大于一个字节*/
      {
